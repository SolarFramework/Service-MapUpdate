...
docker run -d -p %1:8080 -v config_files_mapupdate:/.xpcf -v volume_map:/SolARServiceMapUpdate/data/maps/globalMap -e SERVER_EXTERNAL_URL -e SERVICE_MANAGER_URL -e SOLAR_LOG_LEVEL -e "SERVICE_NAME=SolARServiceMapUpdate" --log-opt max-size=50m -e "SERVICE_TAGS=SolAR" --name solarservicemapupdate artwin/solar/services/map-update-service:latest
